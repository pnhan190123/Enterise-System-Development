  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <form class="w-100" action="{{ route('ykienReporter.statusShow', [$item->idYKien, $AnHien]) }}" method="post">

          {{ csrf_field() }}
          <button type="submit" class="btn w-100 text-center btn-{{ ($item->AnHien==1) ? "primary" : "dark" }}">
            {{ ($item->AnHien==1) ? "Đang hiện" : "Ẩn" }}
          </button>
        </form>
      </td>
