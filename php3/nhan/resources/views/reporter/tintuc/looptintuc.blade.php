    ...
    @foreach ($ds as $item)
      <tr>
        ...
        <td style="white-space:normal">
            ...
            <a href="{{ route('tintucReporter.edit', $item->idTin) }}" class="btn btn-primary">Chỉnh</a>
            <form action="{{ route('tintucReporter.destroy', $item->idTin) }}" method="post" style="display: inline-block;">
                {{ csrf_field() }}
                {!! method_field('delete') !!}
