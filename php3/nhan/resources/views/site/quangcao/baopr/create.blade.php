...
<form method="post" action="{{ route('quangcaoUser.storelist') }}">
    {{ csrf_field() }}
    <div class="container">
    <center>
            <h1>
        ...
        <div class="d-flex justify-content-around">
            ...
            @php
                $kq = DB::select('select id, ten_loai, gialoai from loaibaibao');
            @endphp
            @foreach($kq as $value)
            <div class="d-flex form-group mg-b-20">
                <label class="rdiobox">
                    <input type="radio" name="id_loaibaibao" value="{{$value->id}}" required>
        ...
        <br>
        <input type="checkbox"> Tôi xác nhận đồng ý với tất cả điều khoản và hợp đồng trên
        <br>
        <br>
