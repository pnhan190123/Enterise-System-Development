  ...
  <tbody>
    ...
    <tr>
      ...
      <td><?php
        ...
        <?php } else { ?>
          <a href="{{ route('quangcaoSecretariat.edit', $item->id_quangcao) }}" class="btn btn-primary">Chỉnh</a>
          <form action="{{ route('quangcaoSecretariat.destroy', $item->id_quangcao) }}" method="post">
            {{ csrf_field() }}
            {!! method_field('delete') !!}
