    ...
    @foreach ($ds as $item)
      <tr>
        ...
        <td>
            ...
            @endphp
            <form class="w-100" action="{{ route('ykienSecretariat.statusShow', [$item->idYKien, $AnHien]) }}" method="post">
                {{ csrf_field() }}
        ...
        <td>
            <form action="{{ route('ykienSecretariat.destroy', $item->idYKien) }}" method="post">
                {{ csrf_field() }}
                {!! method_field('delete') !!}
