...
<table id="datatable1" class="table display responsive nowrap">
    ...
    ?>
        ...
        <td>
            ...
            @endphp
            <form class="w-100" action="{{ route('userSecretariat.active', [$item->idUser, $active]) }}" method="post">
                {{ csrf_field() }}
                <button type="submit" class="btn w-100 text-center btn-{{ ($item->active==1) ? "primary" : "dark" }}">{{ ($item->active==1) ? "Thăng chức" : "Hạ chức" }}</button>
            </form>
        </td>
        <td>
            <form action="{{ route('userSecretariat.destroy', $item->idUser) }}" method="post">
                {{ csrf_field() }}
                {!! method_field('delete') !!}
