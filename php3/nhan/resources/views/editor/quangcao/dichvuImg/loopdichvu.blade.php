...
<table id="datatable1" class="table display responsive nowrap">
    ...
    @foreach ($ds as $item)
      <tr>
        ...
        <td>{{ $item->ten_loai}}</td>
        <td>{{ $item->gialoai}}$</td>
        <td>{{ ($item->thoigian)/86400}} ngày</td>

        <td>
            @php
                ($item->AnHien==1) ? $AnHien = 0 : $AnHien = 1
            @endphp
            <form class="w-100" action="{{ route('quangcaoEditor.changeStatusShowlist', [$item->id, $AnHien]) }}" method="post">
                {{ csrf_field() }}
        ...
        <td>
            <a href="{{ route('quangcaoEditor.editlist', $item->id) }}" class="btn btn-primary">Chỉnh</a>
        </td>
        <td>
            <form action="{{ route('quangcaoEditor.destroylist', $item->id) }}" method="post">
                {{ csrf_field() }}
                {!! method_field('delete') !!}
